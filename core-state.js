...
const APP_GANG_FACTION = "Cawdor";

let appState = { view: 'menu', mode: null, editTarget: null, isQuickMatch: false, returnTo: null };
let savedGangs = JSON.parse(localStorage.getItem('cawdorGangs')) || {};
...
function migrateSavedGangsIfNeeded() {
    const storedVersion = parseInt(localStorage.getItem('cawdorGangsSchemaVersion'), 10) || 0;
    ...
    localStorage.setItem('cawdorGangsSchemaVersion', String(GANGS_SCHEMA_VERSION));
    saveGangs();
}
...
// (cawdorGangs_otherFactions) : aucune perte de données, la liste étrangère reste
...
function quarantineForeignFactionGangs() {
    ...
    let quarantine = {};
    try {
        quarantine = JSON.parse(localStorage.getItem('cawdorGangs_otherFactions')) || {};
    } catch (e) {
        quarantine = {};
    }
    ...
    localStorage.setItem('cawdorGangs_otherFactions', JSON.stringify(quarantine));
...
function saveGangs() {
    ...
    localStorage.setItem('cawdorGangs', JSON.stringify(savedGangs));
}
...
const FIGHTER_TYPE_DESCRIPTIONS = {
    ...
    'pious': "Si ce guerrier est le leader du gang, l'armée est Pious : accès aux armes/équipements marqués Pious, règle Without number (un Prospect additionnel en sélection de bataille), et jamais plus de guerriers Fanatic que de guerriers Pious dans le roster recruté.",
    'fanatic': "Si ce guerrier est le leader du gang, l'armée est Fanatic : accès aux armes/équipements marqués Fanatic, règle Fanatical (relance des bottle checks dont le résultat total est 11 ou 12), et jamais plus de guerriers Pious que de guerriers Fanatic dans le roster recruté."
};

// Alignement de l'armée (Pious ou Fanatic), déterminé uniquement par le type
// porté par le LEADER actuel du gang (voir règle spéciale d'armée). Retourne
// null tant qu'aucun leader n'est recruté (aucun accès pious/fanatic ouvert).
function getGangAlignment(gang) {
    gang = gang || currentGang;
    if (!gang || !gang.members) return null;
    let leader = gang.members.find(m => (m.type || []).some(t => normalizeTypeKey(t) === 'leader'));
    if (!leader) return null;
    if ((leader.type || []).some(t => normalizeTypeKey(t) === 'pious')) return 'pious';
    if ((leader.type || []).some(t => normalizeTypeKey(t) === 'fanatic')) return 'fanatic';
    return null;
}

// Alignement (Pious ou Fanatic) d'UN guerrier en particulier, d'après son
// propre type (celui du personnage recruté) — indépendant de l'alignement
// de l'ARMÉE (déterminé par le leader, voir getGangAlignment). L'accès aux
// armes/équipements pious/fanatic se vérifie toujours sur ce type individuel :
// un guerrier pious garde son accès pious même dans une armée fanatic, et
// inversement. Fonctionne dès la sélection du profil, avant même la
// sauvegarde de la fiche (donc aussi avant qu'un leader existe).
function getFighterAlignment(fighter) {
    if (!fighter) return null;
    let types = (fighter.type || []).map(t => normalizeTypeKey(t));
    if ((!types || types.length === 0) && fighter.charId && typeof db !== 'undefined' && db.characters) {
        let charDef = db.characters.find(c => c.id === fighter.charId);
        if (charDef) types = (charDef.type || []).map(t => normalizeTypeKey(t));
    }
    if (types.includes('pious')) return 'pious';
    if (types.includes('fanatic')) return 'fanatic';
    return null;
}

// Accès d'un item (arme, équipement ou familier-équipement) à la liste de
// recrutement directe du gang, selon l'alignement pious/fanatic courant.
// N'affecte jamais l'accès mercenaire (specific_to) ni hive scum (is_hive_scum),
// gérés séparément et inchangés.
function hasFactionAccess(item, alignment) {
    if (!item || !alignment) return false;
    if (alignment === 'pious') return item.is_weapon_pious === true || item.is_equip_pious === true;
    if (alignment === 'fanatic') return item.is_weapon_fanatic === true || item.is_equip_fanatic === true;
    return false;
}

// Comptage pious/fanatic du roster actuel (hors mercenaires/bêtes/familiers,
// qui ne portent pas ces types). Sert à faire respecter la règle "jamais plus
// de l'alignement opposé que de l'alignement dominant".
function countPiousFanaticInRoster(gang) {
    gang = gang || currentGang;
    let counts = { pious: 0, fanatic: 0 };
    ((gang && gang.members) || []).forEach(m => {
        let types = (m.type || []).map(t => normalizeTypeKey(t));
        if (types.includes('pious')) counts.pious++;
        if (types.includes('fanatic')) counts.fanatic++;
    });
    return counts;
}
...
function getEffectiveFighterTypes(m) {
    ...
    let isMounted = (m.equipment || []).some(e => e && (e.id === 'eq_dirt_bike' || e.id === 'eq_ridge_walker'));
...
function checkMountedWeaponRestriction(fighter, weapon) {
    ...
    let isMounted = (fighter.equipment || []).some(e => e && (e.id === 'eq_dirt_bike' || e.id === 'eq_ridge_walker'));
...
// relatifs) : Ridge walker -> 8", Dirt bike -> 8". Si les deux sont équipés
// en même temps (non prévu par les règles), le Ridge walker prévaut.
...
function getMountedMOverride(m) {
    ...
    if (m.equipment.some(e => e && e.id === 'eq_ridge_walker')) return MOUNTED_M_OVERRIDES['eq_ridge_walker'];
