...
const SCENARIO_TYPES = {
    intensification: {
        ...
        desc: "Choisissez 3 guerriers. Choisissez un nombre de guerriers (1 à 3, à convenir avec votre adversaire pour que les deux gangs aient le même) que le programme tirera ensuite au hasard. Vous pourrez ensuite choisir jusqu'à 5 renforts. D3 renforts arriveront en jeu à partir du second round."
    },
    donnez_tout: {
        name: "Donnez tout !",
    ...
    force_reconnaissance: {
        ...
        desc: "Choisissez un nombre de guerriers à sélectionner (1 à 3, à convenir avec votre adversaire pour que les deux gangs aient le même), puis ajoute 5 guerriers au hasard."
    }
};
...
function resetSetupState() {
    // Bonus "Without number" : automatique et non désactivable dès lors que le
    // gang est Pious (déterminé par le type du leader, voir getGangAlignment).
    // Un gang Fanatic n'y a jamais accès (son bonus d'armée est différent :
    // relance des bottle checks à 11-12, indiqué sur les cartes des guerriers).
    let _autoAlignment = (typeof getGangAlignment === 'function') ? getGangAlignment(currentGang) : null;
    setupState = {
        scenarioKey: 'intensification',
        role: 'attacker',
...
function hasMountedEquipment(m) {
    return !!(m && (m.equipment || []).some(e => e && (e.id === 'eq_dirt_bike' || e.id === 'eq_ridge_walker')));
}
    ...
    let html = `
        <div class="card">
            ...
            ${setupState.scenarioKey === 'force_reconnaissance' ? `
                <div style="margin-bottom:12px; background:#111; padding:10px 12px; border-radius:5px;">
                    <label style="font-weight:bold; display:block; margin-bottom:6px;">Nombre de guerriers à choisir (à convenir avec l'adversaire, pour que les deux gangs aient le même) :</label>
                    <div style="display:flex; gap:8px;">
                        ${[1, 2, 3].map(n => `
                            <button class="btn ${setupState.reconCount === n ? 'btn-cyan' : ''}" style="flex:1; padding:8px;" onclick="setReconCount(${n})">${n}</button>
                        `).join('')}
                    </div>
                </div>
            ` : ''}
            ...
            ${setupState.scenarioKey === 'intensification' ? `
                <div style="margin-bottom:12px; background:#111; padding:10px 12px; border-radius:5px;">
                    <label style="font-weight:bold; display:block; margin-bottom:6px;">Nombre de guerriers que le programme tirera au hasard (à convenir avec l'adversaire, pour que les deux gangs aient le même) :</label>
                    <div style="display:flex; gap:8px;">
                        ${[1, 2, 3].map(n => `
                            <button class="btn ${setupState.intensificationRandomCount === n ? 'btn-cyan' : ''}" style="flex:1; padding:8px;" onclick="setIntensificationRandomCount(${n})">${n}</button>
                        `).join('')}
                    </div>
                </div>
            ` : ''}

            ${setupState.piousActive ? `
                <div style="margin-bottom:15px; background:#111; padding:8px 12px; border-radius:5px; border:1px solid #f39c12;">
                    <strong style="color:#f39c12;">Without number (armée Pious)</strong><br>
                    <small style="color:#aaa;">Le gang peut inclure un Prospect supplémentaire dans son équipe de départ.</small>
                </div>
            ` : ''}
...
function renderStep1View(availableMembers) {
    ...
    if (key === 'intensification') {
        ...
        maxSelect = req;
        labelHelp = `Force de reconnaissance : vous devez choisir ${req} guerrier(s) (nombre convenu avec l'adversaire).`; 
    }
    ...
    availableMembers.forEach(m => {
        if (m.id === setupState.piousPickedId) return;
...
function renderPiousSection(availableMembers) {
    ...
    let isMandatory = (setupState.scenarioKey === 'donnez_tout');
    let html = `
        <div style="background:#221a08; border:1px solid #f39c12; padding:8px; border-radius:5px; margin-top:8px;">
            <strong style="color:#f39c12;">Bonus Pious : Sélectionner ${isMandatory ? '(obligatoire)' : '(optionnel)'} 1 Prospect</strong><br>
    `;
    ...
    if (prospects.length === 0) {
        ...
        html += `
            <select style="width:100%; padding:6px; margin-top:4px; background:#111; color:#fff; border:1px solid #444;" onchange="selectPiousProspect(this.value)">
                <option value="">-- Choisir un Prospect ${isMandatory ? '' : '(Optionnel)'} --</option>
                ${prospects.map(p => `
...
// Nombre de guerriers à choisir pour Force de reconnaissance, et nombre de
// guerriers tirés au hasard pour Intensification de la bataille : choisis
// par le joueur (1 à 3) plutôt que déterminés aléatoirement par le
// programme, pour que les deux adversaires puissent se mettre d'accord sur
// le même nombre avant de jouer.
function setReconCount(n) {
    setupState.reconCount = n;
    setupState.initialPickedIds = [];
    renderGameSetup(document.getElementById('main-content'));
}

function setIntensificationRandomCount(n) {
    setupState.intensificationRandomCount = n;
    renderGameSetup(document.getElementById('main-content'));
}

// L'activation du bonus "Without number" (piousActive) n'est plus manuelle :
// elle est recalculée automatiquement dans resetSetupState() à partir de
// l'alignement du gang (getGangAlignment). Rien à basculer ici.
...
function validateStep1() {
    ...
    if (key === 'intensification') {
        ...
        let nbRandom = Math.min(setupState.intensificationRandomCount || 1, remainingPool.length);
...
function startGame() {
    ...
    let totalInitial = setupState.initialPickedIds.length + (setupState.piousPickedId ? 1 : 0);
    if (totalInitial === 0 && setupState.randomDrawnIds.length === 0) {
        return showToast("Veuillez sélectionner au moins un combattant.", "error");
    }

    // Pour "Donnez tout !", une armée Pious doit obligatoirement inclure son
    // Prospect bonus (Without number) et pas seulement pouvoir le faire.
    if (setupState.scenarioKey === 'donnez_tout' && setupState.piousActive && !setupState.piousPickedId) {
        return showToast("Armée Pious : le Prospect bonus (Without number) est obligatoire pour \"Donnez tout !\".", "error");
    }
    ...
    if (setupState.piousPickedId) {
        addFighterWithFamiliars(setupState.piousPickedId, false);
    }
...
function addFighterToGameRoster(memberObj, isReinforcement) {
    ...
    if (typeof setupState !== 'undefined' && setupState.mountedOptOutIds && setupState.mountedOptOutIds.includes(memberObj.id)) {
        m.equipment = (m.equipment || []).filter(e => !(e && (e.id === 'eq_dirt_bike' || e.id === 'eq_ridge_walker')));
    }
