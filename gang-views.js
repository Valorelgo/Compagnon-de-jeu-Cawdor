...
function renderMenu(container) {
    ...
    updateTopBar();
    container.innerHTML = `
        <div class="cawdor-hero">
            <span class="cawdor-hero-icon">🔥</span>
            <h1>Necromunda — Compagnon Cawdor</h1>
            <p class="subtitle">« Dans les cendres du Sous-Monde, seule la Foi — ou la Fureur — nous montre le chemin. »</p>
        </div>

        <div class="cawdor-menu-grid">
            <button class="cawdor-action-card" onclick="navigate('gang-create')">
                <span class="icon">⚜️</span>
                <span>
                    <span class="label">Fonder un gang</span>
                    <span class="desc">Rassembler une nouvelle congrégation Cawdor et choisir sa voie — Pious ou Fanatic.</span>
                </span>
            </button>

            <button class="cawdor-action-card" onclick="appState.mode='campaign'; navigate('gang-select')">
                <span class="icon">📜</span>
                <span>
                    <span class="label">Suivre une campagne</span>
                    <span class="desc">Reprendre un gang déjà établi : roster, territoires, cartes tactiques et progression.</span>
                </span>
            </button>

            <hr class="cawdor-divider">

            <button class="cawdor-action-card secondary" onclick="importGang()">
                <span class="icon">📥</span>
                <span>
                    <span class="label">Importer un gang Cawdor</span>
                    <span class="desc">Charger une liste sauvegardée au format .json.</span>
                </span>
            </button>
        </div>
    `;
...
function renderGangCreate(container) {
    container.innerHTML = `
        <div class="card">
            <h2>Créer un nouveau gang Cawdor</h2>
...
function ensureInnateFighterSkills(gang) {
    ...
    gang.members.forEach(m => {
        ...
        // Monté (Ridge walker/Dirt bike équipé) -> Nerves of steel et Hit & run.
        ...
        const isMounted = (m.equipment || []).some(e => e && (e.id === 'eq_dirt_bike' || e.id === 'eq_ridge_walker'));
        if (isMounted) {
        ...
        // Armes intégrées données par une compétence (ex. Headbutt) : synchro
        // générique, à faire en tout dernier pour refléter l'état final des
        // compétences après tous les ajouts ci-dessus (Juggernaut, Chef...).
...
function loadGang(name) {
    ...
    if (g.faction && g.faction !== APP_GANG_FACTION) {
        return showToast(`"${name}" est une liste ${g.faction}, pas Cawdor. Ce programme ne peut pas l'ouvrir.`, "error");
    }
    currentGang = g;
