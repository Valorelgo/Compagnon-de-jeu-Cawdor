    ...
    if (charDef && charDef.allowed_merc_weapons) {
        ...
        const availableWeapons = db.weapons.filter(w => {
            ...
            if (appState.returnTo === 'post-cycle' && appState.editTarget !== null) {
                return hasFactionAccess(w, getFighterAlignment(tempFighter));
            }

            let isAllowedExclusive = charDef && charDef.allowed_weapons_exclusive && charDef.allowed_weapons_exclusive.includes(w.id);
            return hasFactionAccess(w, getFighterAlignment(tempFighter)) || w.specific_to === tempFighter.charId || isAllowedExclusive;
        });
...
function openWeaponAccessoryModal(weaponIdx) {
    ...
    let availableAccessories = db.equipment.filter(e => {
        ...
        return hasFactionAccess(e, getFighterAlignment(tempFighter));
    });
...
function openEquipSelectModal() {
    ...
    // ont le tag is_gang (marqueur générique de recrutabilité) ET l'accès
    // pious/fanatic du guerrier ACHETEUR (tempFighter), pas de l'armée entière.
    ...
    let availableFamiliars = (!isMercOrBeast && !isHiveScum && typeof db !== 'undefined' && db.characters)
        ? db.characters.filter(c => typeof isNonMercFamiliarCharDef === 'function' && isNonMercFamiliarCharDef(c) && c.is_gang === true && hasFactionAccess(c, getFighterAlignment(tempFighter)))
        : [];
