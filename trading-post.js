...
function renderTradingPostView() {
    // Au Trading Post, le gang a accès à toute la base commune, y compris les
    // armes/équipements réservés à l'alignement opposé (pious/fanatic) : seuls
    // les items déjà sur SA propre liste de recrutement directe sont exclus
    // (pas besoin de les tirer au Trading Post, ils sont déjà achetables).
    let _tpAlignment = getGangAlignment();
    let dbWeapons = (typeof db !== 'undefined' && db.weapons) ? db.weapons.filter(w => 
        !w.is_merc_weapon && !w.default_for && !w.specific_to && !w.requires_equip && !w.counts_as_equip && w.type !== "Grenade" && !hasFactionAccess(w, _tpAlignment)
    ) : [];

    let dbGrenades = (typeof db !== 'undefined' && db.weapons) ? db.weapons.filter(w => 
        (w.counts_as_equip || w.type === "Grenade") && !w.specific_to && !w.requires_equip && !hasFactionAccess(w, _tpAlignment)
    ) : [];
    
    let dbEquip = (typeof db !== 'undefined' && db.equipment) ? db.equipment.filter(e => 
        !e.specific_to && !hasFactionAccess(e, _tpAlignment)
    ) : [];
