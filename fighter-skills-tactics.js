...
function openSkillModal() {
    ...
    // Une carte par compétence (fond + bordure), cohérente entre les deux
    // variantes de ce menu (Spécialiste / catégories classiques).
    const buildSkillCardHTML = (s, cat, isChecked, isAllowed, onchangeAttr, extraLabel) => `
        <div class="skill-checkbox-group" style="padding:8px 10px; background:#181818; border-radius:5px; border:1px solid #333; ${isAllowed ? '' : 'opacity:0.4;'}">
            <input type="checkbox" id="sk-${s.id}" ${isChecked ? 'checked' : ''} ${isAllowed ? '' : 'disabled'} ${onchangeAttr}>
            <label for="sk-${s.id}"><strong>${s.name}</strong>${extraLabel ? ` <small style="color:#888;">${extraLabel}</small>` : ''} : ${s.desc}</label>
        </div>
    `;

    let html = `<div style="max-height:72vh; overflow-y:auto; padding-right:6px;">`;
    ...
    if (isProspectOrBeast) {
        ...
        for (let cat in db.skills) {
            db.skills[cat].forEach(s => {
                if (specialistSkillIds.includes(s.id)) {
                    ...
                    html += buildSkillCardHTML(s, cat, isChecked, true, `onchange="toggleSpecialistSkill('${s.id}', '${cat}')"`, `(${cat.toUpperCase()})`);
                }
            });
        }
        html += `</div>`;
    } else {
        html += `<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(340px, 1fr)); gap:16px; align-items:start;">`;
        ...
        for (let cat in db.skills) {
            ...
            html += `
                <div class="skill-category" style="margin-bottom:0; ${isAllowed ? '' : 'opacity:0.4; pointer-events:none; filter:grayscale(1);'}">
                    <h4 style="margin-bottom:8px; color:${isPrimary ? 'var(--accent-cyan)' : (isSecondary ? 'var(--accent-purple)' : '#888')};">
            ...
            db.skills[cat].forEach(s => {
                ...
                let isChecked = tempFighter.skills.some(sk => sk.id === s.id);
                html += buildSkillCardHTML(s, cat, isChecked, isAllowed, `onchange="toggleSkill('${cat}', '${s.id}')"`, null);
            });
...
function saveFighter() {
    ...
    // Règle spéciale d'armée : jamais plus de guerriers de l'alignement opposé
    // (pious/fanatic) que de l'alignement du leader dans le roster recruté.
    // Ne s'applique qu'à une nouvelle recrue (pas à l'édition d'un combattant
    // déjà présent, qui ne change pas son propre alignement).
    if (appState.editTarget === null && typeof getGangAlignment === 'function') {
        let alignment = getGangAlignment(currentGang);
        let candidateTypes = (tempFighter.type || []).map(t => normalizeTypeKey(t));
        let addsPious = candidateTypes.includes('pious');
        let addsFanatic = candidateTypes.includes('fanatic');
        if (alignment && (addsPious || addsFanatic)) {
            let counts = countPiousFanaticInRoster(currentGang);
            let futurePious = counts.pious + (addsPious ? 1 : 0);
            let futureFanatic = counts.fanatic + (addsFanatic ? 1 : 0);
            if (alignment === 'pious' && futureFanatic > futurePious) {
                return showToast(`Armée Pious : impossible d'avoir plus de guerriers Fanatic (${futureFanatic}) que de guerriers Pious (${futurePious}) dans le roster.`, "error");
            }
            if (alignment === 'fanatic' && futurePious > futureFanatic) {
                return showToast(`Armée Fanatic : impossible d'avoir plus de guerriers Pious (${futurePious}) que de guerriers Fanatic (${futureFanatic}) dans le roster.`, "error");
            }
        }
    }
    ...
    // branches confondues) : utilisé pour le message de fin, et pour ne jamais
    // planter si le message est affiché après une branche qui ne définit pas
    // creditsToPay (l'ancien bug "creditsToPay is not defined").
    let totalChargedThisSave = 0;

    if (!currentGang.isEstablished) {
        // Certains objets (ex: familiers, voir buyFamiliarForFighter) sont payés
        // immédiatement au moment de l'achat, pas différés comme le reste : il ne
        // faut pas les refacturer une seconde fois ici via le delta de totalCost.
        const sumPrepaidCost = fighter => {
            let sum = 0;
            (fighter.weapons || []).forEach(w => {
                if (w && w.costPrepaid) sum += (w.cost_credits || w.cost || 0);
                if (w && w.accessory && w.accessory.costPrepaid) sum += (w.accessory.cost_credits || w.accessory.cost || 0);
            });
            (fighter.equipment || []).forEach(e => {
                if (e && e.costPrepaid) sum += (e.cost_credits || e.cost || 0);
            });
            return sum;
        };
        ...
        if (appState.editTarget !== null) {
            ...
            oldCost = origFighter.totalCost;
            oldPrepaid = sumPrepaidCost(origFighter);
        }
        let newPrepaid = sumPrepaidCost(tempFighter);
        let diff = (tempFighter.totalCost - newPrepaid) - (oldCost - oldPrepaid);

        if (currentGang.credits - diff < 0) return showToast("Crédits insuffisants !", "error");
        currentGang.credits -= diff;
        totalChargedThisSave = diff;
    } else {
        let creditsToPay = 0;
        ...
        totalChargedThisSave = creditsToPay;
    }
    ...
    if (wasRecruiting) {
        ...
        if (typeof showToast === 'function') {
            showToast(`Équipement mis à jour (${totalChargedThisSave || 0} cr) !`, "success");
        }
    }
}
...
function removeFighter(idx) {
    ...
    // Pendant la création du gang (pas encore validé), licencier un guerrier
    // annule simplement son recrutement : tout est remboursé (guerrier, armes,
    // équipement, familier compris), rien ne part au stash, qui n'existe pas
    // encore à ce stade. Une fois le gang validé, comportement inchangé
    // ci-dessous : aucun remboursement, matériel envoyé au stash.
    if (!currentGang.isEstablished) {
        let refundAmount = (typeof calculateFighterCost === 'function') ? calculateFighterCost(m) : (m.totalCost || 0);
        showConfirmModal(
            "Retirer le combattant",
            `Voulez-vous vraiment retirer <strong>${m.customName}</strong> (${m.charName}) du gang ?<br><br>
            <small style="color:#2ecc71; font-size:13px;">Le gang n'est pas encore validé : son coût total (${refundAmount} cr, armes et équipement compris) sera intégralement remboursé.</small>`,
            "Retirer",
            () => {
                performRemoveFighterDuringCreation(idx);
            }
        );
        return;
    }
...
// Retrait pendant la création du gang : remboursement intégral, rien ne va au
// stash (pas encore de stash à ce stade). Un éventuel familier rattaché à ce
// guerrier est retiré du roster (déjà remboursé via le coût total ci-dessus,
// puisque sa valeur est comptée comme équipement chez son propriétaire — pas
// de double remboursement).
function performRemoveFighterDuringCreation(idx) {
    const m = currentGang.members[idx];
    if (!m) return;

    let refundAmount = (typeof calculateFighterCost === 'function') ? calculateFighterCost(m) : (m.totalCost || 0);
    currentGang.credits = (currentGang.credits || 0) + refundAmount;

    if (typeof getFamiliarsOfFighter === 'function') {
        getFamiliarsOfFighter(m.id).forEach(fam => {
            currentGang.members = currentGang.members.filter(x => x.id !== fam.id);
        });
    }
    ...
    showToast(`${m.customName} a été retiré(e) du gang. Coût remboursé intégralement (+${refundAmount} cr).`, "success");
}
    ...
    input.onchange = e => {
        ...
        reader.onload = (event) => {
            try {
                ...
                // Ce programme ne gère que des listes Cawdor : on rejette tout fichier
                // explicitement tagué pour une autre faction (Escher, Delaque,
                // Genestealer...). Un fichier SANS tag (export fait avant l'ajout de cette
                // protection) est accepté et tagué Cawdor, par compatibilité.
                if (gang.faction && gang.faction !== APP_GANG_FACTION) {
                    showToast(`Ce fichier est une liste "${gang.faction}", pas une liste Cawdor. Ce programme n'importe que des listes Cawdor.`, "error");
                    return;
                }
                ...
                saveGangs();
                showToast("Gang Cawdor importé avec succès !", "success");
...
function initApp() {
    ...
    if (typeof _quarantinedGangsCount !== 'undefined' && _quarantinedGangsCount > 0) {
        ...
        showToast(`${n} liste${n > 1 ? 's' : ''} d'une autre faction que Cawdor détectée${n > 1 ? 's' : ''} et retirée${n > 1 ? 's' : ''} de cette appli (conservée${n > 1 ? 's' : ''}, non affichée${n > 1 ? 's' : ''} ici).`, "info");
